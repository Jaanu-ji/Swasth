...
import { getFamilyMembers, addFamilyMember, updateFamilyMember, deleteFamilyMember } from '../../config/api';
...
export default function FamilyScreen() {
  ...
  const loadMembers = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to load family members. Please try again.';
  ...
  const openEditModal = (member) => {
    ...
    setFormData({
      ...
      conditions: Array.isArray(member.diseases) ? member.diseases.join(', ') : '',
      medications: Array.isArray(member.medications) ? member.medications.join(', ') : '',
    });
  ...
  const handleSave = async () => {
    ...
    if (formData.age && (isNaN(formData.age) || parseFloat(formData.age) < 0 || parseFloat(formData.age) > 150)) {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to save family member. Please try again.';
      Alert.alert('Error', errorMessage);
    } finally {
      setSaving(false);
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to delete family member. Please try again.';
      Alert.alert('Error', errorMessage);
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          <View style={styles.content}>
            ...
            <View style={styles.statsGrid}>
              <LinearGradient
                colors={[figmaTokens.colors.blue500, figmaTokens.colors.blue600]}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
              ...
              <LinearGradient
                colors={[figmaTokens.colors.green500, figmaTokens.colors.green600]}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
                style={styles.statCard}
              >
              ...
              <LinearGradient
                colors={[figmaTokens.colors.orange500, figmaTokens.colors.orange600]}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
                style={styles.statCard}
              >
                ...
                <View style={styles.memberInfo}>
                  <View style={styles.memberHeader}>
                    ...
                    <View style={styles.healthyBadge}>
                      <Text style={styles.healthyText}>You</Text>
                    </View>
                  </View>
            ...
            {loading ? (
              ...
              <FigmaCard style={styles.errorCard}>
                <MaterialCommunityIcons name="alert-circle" size={48} color={figmaTokens.colors.red500} />
              ...
              <FigmaCard style={styles.emptyCard}>
                <MaterialCommunityIcons name="account-group" size={64} color={figmaTokens.colors.gray400} />
                <Text style={styles.emptyTitle}>No Family Members</Text>
                <Text style={styles.emptyText}>Add family members to track their health records</Text>
                <FigmaButton
              ...
              <View style={styles.membersList}>
                {members.map((member) => (
                  <TouchableOpacity
                    ...
                    onLongPress={() => handleDelete(member._id)}
                  >
                    <View style={styles.memberContent}>
                      <View style={styles.avatarContainer}>
                        <Text style={styles.avatarText}>
                        ...
                        <View style={styles.memberHeader}>
                          ...
                          {member.status === 'vaccination-due' && (
                            <View style={styles.alertBadge}>
                              <Text style={styles.alertText}>Vaccination Due</Text>
                            </View>
                          )}
                        </View>
                        <Text style={styles.memberDetails}>
                          {member.relationship} • {member.age ? `${member.age} years` : 'Age not set'}
                        </Text>
                        <Text style={styles.memberDate}>
                          Last checkup: {member.updatedAt ? new Date(member.updatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'Never'}
                        </Text>
                      </View>
                      <MaterialCommunityIcons name="chevron-right" size={20} color={figmaTokens.colors.gray400} />
                    </View>
          ...
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => setShowAddModal(false)}>
                <MaterialCommunityIcons name="close" size={24} color={figmaTokens.colors.gray700} />
              </TouchableOpacity>
            </View>
      ...
      <Portal>
        <Dialog visible={showDeleteDialog} onDismiss={() => setShowDeleteDialog(false)} style={styles.dialog}>
          <Dialog.Title style={styles.dialogTitle}>Delete Family Member</Dialog.Title>
          <Dialog.Content>
          ...
          <Dialog.Actions style={styles.dialogActions}>
            ...
            >
              {deleting ? (
                <ActivityIndicator size="small" color={figmaTokens.colors.destructive} />
              ) : (
